...
class SharedFlowScenarioTest : TestBase() {
    ...
    fun testReplay1Extra2() =
        testSharedFlow<Int>(MutableSharedFlow(1, 2)) {
            // total buffer size == 3
            expectReplayOf()
    ...
    @Test
    fun testReplay1InitialValue() =
        testSharedFlow(MutableSharedFlow(1, initialValue = 0)) {
            expectReplayOf(0)
            emitRightNow(1); expectReplayOf(1)
            emitRightNow(2); expectReplayOf(2)
            sharedFlow.resetReplayCache(); expectReplayOf(0)
            sharedFlow.resetReplayCache(); expectReplayOf(0)
            emitRightNow(3); expectReplayOf(3)
            ...
            collect(a, 6, 0); emitResumes(e7); expectReplayOf(7)
            sharedFlow.resetReplayCache(); expectReplayOf(0)
            sharedFlow.resetReplayCache(); expectReplayOf(0) // buffer 7 | 0 -- extra item buffered 'cause reset
            val b = subscribe("b")
            collect(b, 0)
            collect(a, 7) // buffer 0
            collect(a, 0); emitResumes(e8); expectReplayOf(8)
            ...
            sharedFlow.resetReplayCache(); expectReplayOf(0)
            sharedFlow.resetReplayCache(); expectReplayOf(0)
            val e13 = emitSuspends(13)
            val e14 = emitSuspends(14)
            val e15 = emitSuspends(15)
            collect(b, 0)
            cancel(e13)
            cancel(a); emitResumes(e14); expectReplayOf(14)
            collect(b, 14); emitResumes(e15); expectReplayOf(15)
            collect(b, 15)
        }

    @Test
    fun testReplay2Extra2DropOldestInitialValue() =
        testSharedFlow<Int>(MutableSharedFlow(2, 2, BufferOverflow.DROP_OLDEST, 0)) {
            expectReplayOf(0)
            ...
            sharedFlow.resetReplayCache(); expectReplayOf(0) // buffer 12, 13 | 14, 0 // "a" missed 11
            sharedFlow.resetReplayCache(); expectReplayOf(0)
            collect(a, 12, 13, 14, 0)
            collect(b, 0)
        }
